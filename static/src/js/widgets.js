function openerp_picking_widgets(instance){
    ...
    module.PickingEditorWidget = instance.web.Widget.extend({
        ...
        renderElement: function(){
            ...
            this.$('.oe_scanbox').keydown(function(e) {
                if (e.keyCode == 13) {
                    //$(this).closest('form').submit();
                    self.$('#btnScan2')[0].click();
                }
            });
            this.$('.js_scan_new').click(function(){
                var ean128 = self.$('.oe_scanbox').val();
                //alert("Linea 157");
                //self.getParent().barcode_scanner.connect2(function(ean){
                  //  self.get_Parent().scan(ean);
                //});
                self.getParent().scan(ean128);
                //self.getParent().scan_product_id(3,true,'101');
            });
            self.$('.oe_scanbox').focus();
            //self.$("#txtBarcode.form-control.oe_scanbox").focus();
            ...
            this.$('.js_validate_pack').click(function(){
                ...
                var ean128 = self.$('.js_ean_128').val();
                ...
                if (ean128 !== '')
                    self.getParent().set_package_ean128(pack_id, ean128);
                }
            });
    ...
    module.PickingMainWidget = module.MobileWidget.extend({
            ...
            if( picking_id ){
                var loaded_picking = new instance.web.Model('stock.picking')
                    ...
                    .then(function(picking){
                        ...
                        //alert("Linea 706. load_picking_list.");
        ...
        set_package_ean128: function(package_id, ean128){
            var self = this;
            return new instance.web.Model('stock.quant.package').call('write',[[package_id],{'ean128': ean128 }]);
        },
    ...
    module.BarcodeScanner = instance.web.Class.extend({
        ...
        connect2: function(callback){
            var code = "7707210609780";
            //self.get_Parent().scan(code);
            //$('body').off('keypress', this.handler);
            //alert("Linea 1109, antes de callback " + callback);
            callback(code);
            //alert("Linea 1111, despues de callback " + callback);
        },
    });
